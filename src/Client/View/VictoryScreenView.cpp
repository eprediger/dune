...
VictoryScreenView::VictoryScreenView(const int width, const int height,
                                     std::string winner_player) :
    View(width, height),
    backgroundImage(Path::rootVar("assets/img/bkgr/dune2000_1.jpg"),
                    this->window),
    ...
    author_tag("Authors: Grupo 2 [Perez Ondarts Flavio - Poggio Demian - Prediger Emiliano]",
                TXT_FONT_SIZE, this->window),
...
void VictoryScreenView::render() {
    ...
    this->winner_tag.render((this->window.width / 4),
                        (this->window.height - this->winner_tag.textHeight) / 4
                        + winner_tag.textHeight);
    this->thanks_tag.render((this->window.width / 4),
                        (this->window.height - this->thanks_tag.textHeight) / 2
                        + thanks_tag.textHeight);
    this->author_tag.render((this->window.width - this->author_tag.textWidth)/2,
    (this->window.height - this->author_tag.textHeight) * 8 / 10
     + author_tag.textHeight);
